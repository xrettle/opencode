...
func (g *geminiClient) send(ctx context.Context, messages []message.Message, tools []tools.BaseTool) (*ProviderResponse, error) {
	...
	for {
		...
		if len(resp.Candidates) > 0 && resp.Candidates[0].Content != nil {
			...
			// Extract tool calls using the toolCalls method
			toolCalls = g.toolCalls(resp)
			// Mark all tool calls as finished in the send method
		...
		for {
			...
			for resp, err := range chat.SendMessageStream(ctx, lastMsgParts...) {
				...
				if len(resp.Candidates) > 0 && resp.Candidates[0].Content != nil {
					...
					// Extract tool calls using the toolCalls method
					newToolCalls := g.toolCalls(resp)
					
					// Mark all tool calls as finished
					for i := range newToolCalls {
						newToolCalls[i].Finished = true
					}
					
					// Add only new tool calls to avoid duplicates
					for _, newCall := range newToolCalls {
						isNew := true
