...
Jev เป็นโมเดล System One จาก TypeSafe AI สำหรับการตัดสินใจที่รวดเร็วและมีโครงสร้าง แทนที่จะสร้างข้อความ โมเดลจะประเมิน `state` เทียบกับคำถามที่กำหนดชนิดไว้ แล้วส่งคืนค่าและความน่าจะเป็นที่โค้ดของคุณนำไปใช้ได้โดยตรง รองรับคำถามแบบใช่/ไม่ใช่ (`noul`) แบบหลายตัวเลือก (`choice`) และแบบให้คะแนนตามเกณฑ์ (`score`)

ใช้ API key ของ OpenCode Zen กับ endpoint `https://opencode.ai/zen/v1/systemone` ตัวอย่างนี้ตรวจสอบว่า request ที่ส่งถึงฝ่าย support เป็นเรื่องเร่งด่วนหรือไม่:
...
คุณสามารถถามหลายคำถามได้ใน request เดียว Jev จะประเมินคำถามเหล่านั้นพร้อมกัน และส่งคืนคำตอบแต่ละรายการภายใต้ ID ของคำถามที่ตรงกัน:
...
ใช้ `jev-1.13-free` แทน `jev-1.13` เพื่อใช้โมเดลฟรีในช่วงเวลาจำกัด ดูรายละเอียดเกี่ยวกับชนิดคำถามและฟิลด์ใน response ได้ใน[เอกสาร TypeSafe AI](https://docs.typesafe.ai/)
...
- MiMo-V2.6-Flash Free เปิดให้ใช้บน OpenCode ในช่วงเวลาจำกัด ทีมกำลังใช้ช่วงเวลานี้เพื่อเก็บ feedback และปรับปรุงโมเดล
...
- MiMo-V2.6-Flash Free: ระหว่างช่วงที่เปิดให้ใช้ฟรี ข้อมูลที่เก็บรวบรวมอาจถูกนำไปใช้เพื่อปรับปรุงโมเดล
