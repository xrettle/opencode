...
Jev er en System One-modell fra TypeSafe AI for raske, strukturerte beslutninger. I stedet for å generere tekst evaluerer den en `state` mot typede spørsmål og returnerer verdier og sannsynligheter som koden din kan bruke direkte. Den støtter ja/nei-spørsmål (`noul`), flervalgsspørsmål (`choice`) og kriteriebaserte spørsmål (`score`).

Bruk OpenCode Zen API Nøkkelen din med endepunktet `https://opencode.ai/zen/v1/systemone`. Dette eksempelet sjekker om en støtteforespørsel haster:
...
Du kan stille flere spørsmål i én forespørsel. Jev evaluerer dem parallelt og returnerer hvert svar under den tilhørende spørsmåls-ID-en:
...
Bruk `jev-1.13-free` i stedet for `jev-1.13` for å bruke den tidsbegrensede gratis-modellen. Se [TypeSafe AI-dokumentasjonen](https://docs.typesafe.ai/) for detaljer om spørsmålstyper og svarfelt.
