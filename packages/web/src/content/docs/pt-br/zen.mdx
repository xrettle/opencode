...
Jev é um modelo System One da TypeSafe AI para decisões rápidas e estruturadas. Em vez de gerar texto, ele avalia um `state` com base em perguntas tipadas e retorna valores e probabilidades que seu código pode usar diretamente. Ele é compatível com perguntas de sim/não (`noul`), de múltipla escolha (`choice`) e baseadas em critérios de avaliação (`score`).

Use sua chave de API do OpenCode Zen com o endpoint `https://opencode.ai/zen/v1/systemone`. Este exemplo verifica se uma solicitação de suporte é urgente:
...
Você pode fazer várias perguntas em uma única solicitação. Jev as avalia em paralelo e retorna cada resposta sob o ID da pergunta correspondente:
...
Use `jev-1.13-free` em vez de `jev-1.13` para usar o modelo gratuito por tempo limitado. Consulte a [documentação da TypeSafe AI](https://docs.typesafe.ai/) para saber mais sobre os tipos de perguntas e os campos de resposta.
...
| Jev 1.13 Free                     | Grátis  | Grátis  | -                | -                |
| Jev 1.13                          | $0.042  | Grátis  | -                | -                |
