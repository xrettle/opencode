...
Jev, hızlı ve yapılandırılmış kararlar için TypeSafe AI tarafından geliştirilen bir System One modelidir. Metin üretmek yerine bir `state` değerini türü belirlenmiş sorulara göre değerlendirir ve kodunuzun doğrudan kullanabileceği değerler ile olasılıklar döndürür. Evet/hayır (`noul`), çoktan seçmeli (`choice`) ve ölçüt tabanlı puanlama (`score`) sorularını destekler.

OpenCode Zen API anahtarınızı `https://opencode.ai/zen/v1/systemone` uç noktasıyla kullanın. Bu örnek, bir destek isteğinin acil olup olmadığını denetler:
...
Tek bir istekte birden fazla soru sorabilirsiniz. Jev bunları paralel olarak değerlendirir ve her yanıtı eşleşen soru ID'si altında döndürür:
...
Sınırlı süreli ücretsiz modeli kullanmak için `jev-1.13` yerine `jev-1.13-free` kullanın. Soru türleri ve yanıt alanları hakkında ayrıntılar için [TypeSafe AI belgelerine](https://docs.typesafe.ai/) bakın.
...
| Jev 1.13 Free                     | Ücretsiz | Ücretsiz | -           | -            |
| Jev 1.13                          | $0.042   | Ücretsiz | -           | -            |
...
- MiMo-V2.6-Flash Free, sınırlı bir süre için OpenCode'da ücretsizdir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
...
- MiMo-V2.6-Flash Free: Ücretsiz döneminde toplanan veriler modeli iyileştirmek için kullanılabilir.
